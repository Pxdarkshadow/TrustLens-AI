...
import { Update as UpdateIcon, Person as PersonIcon, QrCode as QrCodeIcon, Settings as SettingsIcon, ShoppingCart as ShoppingCartIcon } from '@mui/icons-material';
...
export const RetailerDashboard = () => {
  const { user, isRetailer, isLoading: authLoading } = useAuth();
  const [productsSold, setProductsSold] = useState(0);
  const [loadingStats, setLoadingStats] = useState(true);

  useEffect(() => {
    if (!authLoading && isRetailer()) {
      // In a real app, this would fetch products sold by this retailer
      setProductsSold(0);
  ...
  const cards = [
    {
      ...
      description: 'Scan QR and add retail event',
      icon: <UpdateIcon />,
      to: '/update-product',
      color: '#3f51b5',
    },
    {
      title: 'Products Sold',
      description: `Track ${productsSold} products sold`,
    ...
    <Container maxWidth="xl">
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Welcome, {user?.username}. Record retail sales and manage inventory.
        </Typography>
      </Box>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <Paper elevation={1} sx={{ p: 3, textAlign: 'center', height: '100%' }}>
              ...
              <Typography variant="h6" sx={{ mb: 1 }}>1. Receive Product</Typography>
              <Typography variant="body2" color="text.secondary">
                Scan QR code when receiving products from supplier
              </Typography>
            </Paper>
          </Grid>
          <Grid item xs={12} md={4}>
            <Paper elevation={1} sx={{ p: 3, textAlign: 'center', height: '100%' }}>
              ...
              <Typography variant="h6" sx={{ mb: 1 }}>2. Record Sale</Typography>
              <Typography variant="body2" color="text.secondary">
                Mark product as sold to consumer with timestamp
              </Typography>
            </Paper>
          </Grid>
          <Grid item xs={12} md={4}>
            <Paper elevation={1} sx={{ p: 3, textAlign: 'center', height: '100%' }}>
              ...
              <Typography variant="h6" sx={{ mb: 1 }}>3. Consumer Verify</Typography>
              <Typography variant="body2" color="text.secondary">
                Consumers can verify authenticity at point of sale
              </Typography>
            </Paper>
          </Grid>
        </Grid>
